...
class APITests(unittest.TestCase):
	...
	def add_tasks(self):
		db.session.add(Task("first", date(2018, 1, 1), 10, date(2018,1,1), 1, 1))
		db.session.commit()
		db.session.add(Task("second", date(2018, 1, 1), 10, date(2018,1,1), 1, 1))
		db.session.commit()

	def test_collection_endpoint_returns_collection_data(self):
		self.add_tasks()
	...
	def test_resources_endpoint_returns_correct_data(self):
		...
		self.assertEquals(response.status_code, 200)
		self.assertEquals(response.mimetype, 'application/json')
		self.assertNotIn(b'first', response.data)
		self.assertIn(b'second', response.data)
